...
from langchain_core.tracers.async_base import AsyncBaseTracer
from langchain_core.tracers.base import BaseTracer
from langchain_core.tracers.schemas import Run

Listener = Union[Callable[[Run], None], Callable[[Run, RunnableConfig], None]]
AsyncListener = Union[Callable[[Run], Awaitable[None]], Callable[[Run, RunnableConfig], Awaitable[None]]]
...
class AsyncRootListenersTracer(AsyncBaseTracer):
    ...
    async def _on_run_create(self, run: Run) -> None:
        ...
        if self._arg_on_start is not None:
            await call_func_with_variable_args(self._arg_on_start, run, self.config)
    ...
    async def _on_run_update(self, run: Run) -> None:
        ...
        if run.error is None:
            if self._arg_on_end is not None:
                await call_func_with_variable_args(self._arg_on_end, run, self.config)
        else:
            if self._arg_on_error is not None:
                await call_func_with_variable_args(self._arg_on_error, run, self.config)
